...
namespace GameName
{
    public class ProcedureLaunch : ProcedureBase
    {
        ...
        {
            GameFrameworkLog.Info("ProcedureLaunch OnEnter");
        ...
        {
            GameFrameworkLog.Info("Init language settings");
            if (GameEntry.Base.EditorResourceMode && GameEntry.Base.EditorLanguage != Language.Unspecified)
            {
                // 编辑器资源模式直接使用 Inspector 上设置的语言
                GameFrameworkLog.Info("current EditorLanguage is '{0}'.", GameEntry.Base.EditorLanguage.ToString());
                GameFrameworkLog.Info("current EditorResourceMode is '{0}'.", GameEntry.Base.EditorResourceMode.ToString());
                return;
            }

            Language language = GameEntry.Localization.Language;
            GameFrameworkLog.Info("current language is '{0}'.", language.ToString());
