    ...
    public static class GaiaLighting
    {
        ...
        public static void LightingSavedSetup(GaiaLightingProfile lightProfile, GaiaLightingProfileValues profile)
        {
            ...
            if (lightProfile.m_multiSceneLightingSupport)
            {
                if (multiObject == null)
                {
                    Debug.LogError("Lighting has not yet been set please go to the Gaia Manager and press step 2. in the Standard tab.");
                }
                else
                {
                    GaiaGlobal savedSettings = multiObject.GetComponent<GaiaGlobal>();
                    if (savedSettings == null)
                    {
                        savedSettings = multiObject.AddComponent<GaiaGlobal>();
                    }

                    savedSettings.m_enableSettingSaving = true;
                    savedSettings.ApplySetting(profile, lightProfile);
                }
            }
            else
            {
                ...
                if (savedSettings != null)
                {
                    savedSettings.m_enableSettingSaving = false;
                }
            }
        }
                    ...
                    case GaiaConstants.EnvironmentTarget.UltraLight:
                        {
                            ...
                            if (waterReflections)
                            {
                                ...
                                GaiaWater.SetWaterReflectionsType(waterReflections, gaiaSettings.m_pipelineProfile.m_activePipelineInstalled, gaiaSettings.m_gaiaWaterProfile, gaiaSettings.m_gaiaWaterProfile.m_waterProfiles[gaiaSettings.m_selectedWaterProfile]);
                            }
                            break;
                        }
