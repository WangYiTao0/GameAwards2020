...
namespace Gaia
{
    ...
    public class GaiaLightingProfileEditor : PWEditor
    {
            ...
            if(m_profile != null)
            {
                ...
                if (m_profile.m_multiSceneLightingSupport)
                {
                    if (m_savedSettings != null)
                    {
                        ...
                        if (m_profile.m_lightingProfiles[m_profile.m_selectedLightingProfileValuesIndex].m_typeOfLighting != m_profileValues.m_typeOfLighting)
                        {
                            m_profileValues = m_profile.m_lightingProfiles[m_profile.m_selectedLightingProfileValuesIndex];
                        }

                        GaiaLightingProfileValues selectdValues = m_savedSettings.LoadSettings(m_profileValues, m_profile, true);
                        bool addProfile = true;
                        int customIndex = 0;
                        foreach (GaiaLightingProfileValues profile in m_profile.m_lightingProfiles)
                        {
                            if (profile.m_typeOfLighting == "Custom")
                            {
                                addProfile = false;
                                break;
                            }
                        ...
                        if (addProfile)
                        {
                            ...
                            newProfile.m_typeOfLighting = "Custom";
                        ...
                        {
                            m_profile.m_lightingProfiles[customIndex] = new GaiaLightingProfileValues();
                            GaiaUtils.CopyFields(selectdValues, m_profile.m_lightingProfiles[customIndex]);
                            m_profile.m_lightingProfiles[customIndex].m_typeOfLighting = "Custom";
                            m_profile.m_selectedLightingProfileValuesIndex = customIndex;
                        }
        ...
        private void LightingProfileSettingsEnabled(bool helpEnabled)
        {
            ...
            //{
            //    m_profileValues = GaiaLightingProfile.GetProfile(m_profile);
            //}
            ...
            //    {
            //        Debug.LogError("Unable to find a lighting profile, is the lighting profile missing from the project?");
            //        m_profile.m_lightingProfile = GaiaConstants.GaiaLightingProfileType.None;
            //        return;
            //    }
            //}
            ...
            else
            {
                if (m_renderPipeline != GaiaConstants.EnvironmentRenderer.HighDefinition)
                {
                    ...
                    if (m_profileValues.m_profileType != GaiaConstants.GaiaLightingProfileType.ProceduralWorldsSky)
                    {
                        ...
                        EditorGUILayout.LabelField("Selected Ambient Mode: " + RenderSettings.ambientMode);
