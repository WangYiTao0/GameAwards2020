...
namespace Gaia
{
    ...
    public class Scanner : MonoBehaviour
    {
        ...
        public GaiaConstants.FeatureType m_featureType = GaiaConstants.FeatureType.Custom;
        ...
        public string SaveScan()
        {
            ...
            //Save preview
            string path = GaiaDirectories.GetStampInstanceDirectory(m_featureType, m_featureName);
            GaiaUtils.CompressToMultiChannelFileImage(path, heightmapCopy, heightmapCopy, heightmapCopy, null, TextureFormat.RGBAFloat, GaiaConstants.ImageFileType.Exr, m_baseLevel);
#if UNITY_EDITOR
            EditorUtility.DisplayProgressBar("Generating Texture", "Compressing texture", 0.5f);
#endif
            GaiaUtils.CompressToSingleChannelFileImage(heightmapCopy.Heights(), path, GaiaConstants.fmtHmTextureFormat, m_exportTextureAlso, false);
            ...
            if (m_exportBytesData)
            {
                ...
                heightmapCopy.SaveToBinaryFile(path);
            }
        ...
        public void LoadGameObject(GameObject go)
        {
...
#if UNITY_EDITOR
            EditorUtility.DisplayProgressBar("Processing Mesh", "Processing mesh data", 0.35f);
#endif
            ...
            m_scanBounds = new Bounds(GetPosition(gameObject, null), new Vector3(m_scanWidth * m_scanResolution, m_scanWidth * m_scanResolution * 0.4f, m_scanDepth * m_scanResolution));
            m_baseLevel = m_scanMap.GetBaseLevel();
