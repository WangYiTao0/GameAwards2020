...
namespace Gaia
{
    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        {
            SpawnerPresetListEditor.DrawListElement_GaiaManager(rect, m_BiomeSpawnersToCreate[index], m_editorUtils, this);
        }
            ...
            if (ButtonLeftAligned("StandardTabButtonCreateTerrain", GUILayout.ExpandWidth(true)))
            {
                ...
                else
                {
                    ...
                    if (!cancel)
                    {
                        ...
                        {
                            stamper.m_autoSpawners[i].isActive = m_BiomeSpawnersToCreate[i].m_isActive;
                        }
        ...
        private void AdvancedPanelExtras(bool helpEnabled)
        {
            ...
            if (m_editorUtils.ButtonAutoIndent("Add Global Weather Controller"))
            {
#if GAIA_PRO_PRESENT
                for (int i = 0; i < m_settings.m_gaiaLightingProfile.m_lightingProfiles.Count; i++)
                {
                    if (m_settings.m_gaiaLightingProfile.m_lightingProfiles[i].m_typeOfLighting.Contains("Procedural Worlds Sky"))
                    {
                        GaiaLighting.LightingSavedSetup(m_settings.m_gaiaLightingProfile, m_settings.m_gaiaLightingProfile.m_lightingProfiles[i]);
                    }
                }

                Selection.activeGameObject = ProceduralWorldsGlobalWeather.AddGlobalWindShader(m_settings.m_windType);
#endif
            }
        }
        ...
        private void AdvancedPanelBiomesAndSpawners(bool helpEnabled)
        {
            ...
            if (m_advancedTabFoldoutBiomes)
            {
                ...
                {
                    GaiaUtils.CreateAsset<BiomePreset>(GaiaDirectories.GetUserBiomeDirectory(), "Biome Preset");
                }
...
#region Gaia Main Function Calls
        ...
        private List<Spawner> CreateBiome(BiomePresetDropdownEntry selectedPresetEntry)
        {
            ...
            foreach (BiomeSpawnerListEntry spawnerListEntry in m_BiomeSpawnersToCreate.Where(x => x.m_spawnerSettings != null))
            {
                ...
                EditorUtility.DisplayProgressBar("Creating Tools", "Creating Biome " + selectedPresetEntry.name, ++currentStep / totalSteps);
                biomeController.m_autoSpawners.Add(new AutoSpawner() { isActive = spawnerListEntry.m_isActive, status = AutoSpawnerStatus.Initial, spawner = spawner });
...
#region Create Step 2 (Player, water, sky etc)
            ...
            else if (playerPrefabName == "FPSController")
            {
                ...
                if (playerPrefab != null)
                {
                    ...
                    if (dynamicLoadedTerrains)
                    {
                        ...
                        if (waitForLoad == null)
                        {
                            playerObj.AddComponent<RigidbodyWaitForTerrainLoad>();
                        }
                    }
#endif
        ...
        GameObject CreateWater()
        {
            ...
            else
            {
                ...
                GaiaWater.GetProfile(m_settings.m_selectedWaterProfile, waterMat, m_settings.m_gaiaWaterProfile, m_settings.m_pipelineProfile.m_activePipelineInstalled, true, false);
            }
#endif
                        ...
                        if (m_editorUtils.Button("UpgradeGaiaTo" + m_settings.m_currentRenderer.ToString()))
                        {
                            if (EditorUtility.DisplayDialog("Upgrading Gaia Pipeline", "You're about to change Gaia to use " + m_settings.m_currentRenderer.ToString() + " render pipeline. Are you sure you want to proceed?", "Yes", "No"))
                            {
                                ...
                                else if (m_settings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.Universal)
                                {
                                    ...
                                    {
                                        EditorUtility.DisplayDialog("Warning", "Universal render pipeline is only available in 2019.3 or higher", "Ok");
                                    }
                                }
                            }
                        }
