...
namespace Gaia
{
    ...
    public class StamperEditor : PWEditor, IPWEditor
    {
        ...
        private int m_selectedStampCategoryID = 0;
...
#region AutoSpawnerList
        ...
        {
            StamperAutoSpawnerListEditor.DrawListHeader(rect, true, m_stamper.m_autoSpawners, m_editorUtils, ref m_stamper.m_settings.m_autoSpawnerArea);
        }
        ...
        {
            StamperAutoMaskExportListEditor.DrawListHeader(rect, true, m_stamper.m_autoMaskExporter, m_editorUtils, ref m_stamper.m_settings.m_autoMaskExportArea);
        }
        ...
        private void DrawStamperMaskListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            MaskListButtonCommand mlbc = ImageMaskListEditor.DrawMaskListElement(rect, index, m_stamper.m_settings.m_imageMasks, ref m_collisionMaskListBeingDrawn, m_editorUtils, Terrain.activeTerrain, GaiaUtils.IsStampOperation(m_stamper.m_settings.m_operation), copiedImageMask, m_imageMaskHeader.normal.background, m_gaiaSettings, SessionManager);
            switch (mlbc)
            {
                ...
                //    m_stamper.m_heightModifier = heightModifier;
                    ...
                    if (m_stamper.m_settings.m_operation != operation)
                    {
                        if (operation == GaiaConstants.FeatureOperation.RaiseHeight || operation == GaiaConstants.FeatureOperation.AddHeight)
                        {
                            ...
                            m_stamper.m_settings.m_baseLevel = 0f;
                            ...
                            m_stamper.transform.position = new Vector3(m_stamper.transform.position.x, Terrain.activeTerrain.terrainData.size.y / 2f, m_stamper.transform.position.z);
                        }
                        if (operation == GaiaConstants.FeatureOperation.LowerHeight || operation == GaiaConstants.FeatureOperation.SubtractHeight)
                        {
                            ...
                            m_stamper.transform.position = new Vector3(m_stamper.transform.position.x, Terrain.activeTerrain.terrainData.size.y / 10f, m_stamper.transform.position.z);
                        }
        ...
        private void ExportTerrain(Terrain terrain)
        {
            ...
            hm.Normalise();
            hm.Flip();
            string path = GaiaDirectories.GetStampDirectory() + Path.DirectorySeparatorChar + m_stampCategoryNames[m_selectedStampCategoryID] + Path.DirectorySeparatorChar + terrain.name;
        ...
        private void DrawStamperControls(bool showHelp)
        {
            ...
            m_editorUtils.Panel("AutoTriggers", DrawAutoTriggers, true);

            DrawUndo(showHelp);
            GUILayout.BeginHorizontal();
            float buttonWidth = (EditorGUIUtility.currentViewWidth -40f) / 2f;
            if (GUILayout.Button(GetLabel("Flatten"), GUILayout.Width(buttonWidth)))
            {
                if (EditorUtility.DisplayDialog("Flatten Terrain tiles ?", "Are you sure you want to flatten terrain tiles - this can not be undone ?", "Yes", "No"))
                {
                    ...
                    if (GaiaUtils.HasDynamicLoadedTerrains())
                    {
                        ...
                        {
                            foreach (TerrainScene ts in GaiaTerrainLoaderManager.TerrainScenes)
                            {
            ...
            if (GUILayout.Button(GetLabel("Preview"), GUILayout.Width(buttonWidth)))
            {
            ...
            if (m_stamper.m_autoSpawnRequested || m_stamper.m_heightUpdateRequested)
            {
                ...
                GUI.enabled = true;
                if (GUILayout.Button(GetLabel("Cancel"), GUILayout.Width(buttonWidth)))
                {
            ...
            else
            {
                ...
                if (GUILayout.Button(GetLabel("Stamp"), GUILayout.Width(buttonWidth)))
                {
                    ////Check for potential missing resources in autospawners first
                    //foreach (AutoSpawner autoSpawner in m_stamper.m_autoSpawners)
                    //{
                    //    if (autoSpawner != null && autoSpawner.spawner != null && autoSpawner.spawner.enabled && autoSpawner.spawner.CheckForMissingResources(true))
                    //    {
        ...
        private void DrawMaskBaking(bool showHelp)
        {
            ...
            if (GUILayout.Button(GetLabel("Export Mask")))
            {
                ...
                saveFilePath.Replace(".exr", "");
                ImageProcessing.BakeMaskStack(m_stamper.m_settings.m_imageMasks, currentTerrain, m_stamper.transform, m_stamper.m_settings.m_width * widthfactor, m_maskBakingResolution, saveFilePath);
                AssetDatabase.SaveAssets();
            ...
            m_selectedStampCategoryID = EditorGUILayout.IntPopup(m_editorUtils.GetTextValue("TerrainExportDirectory"), m_selectedStampCategoryID, m_stampCategoryNames.ToArray(), m_stampCategoryIDArray);
            ...
            if (m_editorUtils.Button("ExportTerrain"))
            {
                ...
                if (GaiaUtils.HasDynamicLoadedTerrains())
                {
                    GaiaUtils.CallFunctionOnDynamicLoadedTerrains(ExportTerrain, false);
                    terrainCount = GaiaTerrainLoaderManager.TerrainScenes.Count;
                }
                else
                {
                ...
                Debug.Log(String.Format(m_editorUtils.GetTextValue("ExportTerrainLog"), terrainCount.ToString(), m_stampCategoryNames[m_selectedStampCategoryID]));
            }
            EditorGUILayout.EndHorizontal();
        }
        ...
        private void DrawOperation(bool showHelp)
        {
            ...
            int selectedIndex = EditorGUILayout.Popup(GetLabel("Operation Type"), GaiaConstants.FeatureOperationNames.Select((x, i) => new { item = x, index = i }).First(x=>x.item.Value==(int)operation).index, GaiaConstants.FeatureOperationNames.Select(x=>x.Key).ToArray());
            ...
            {
                GaiaWater.UpdateWaterSeaLevel(m_gaiaSettings.m_gaiaWaterProfile.m_waterPrefab, newSeaLEvel);
