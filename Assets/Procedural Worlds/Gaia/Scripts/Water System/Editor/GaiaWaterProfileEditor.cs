    ...
    public class GaiaWaterProfileEditor : PWEditor
    {
        ...
        private void WaterProfileSettingsEnabled(bool helpEnabled)
        {
            ...
            if (m_gaiaOceanMat != null)
            {
                ...
                else if (m_profile.m_selectedProfile != m_profileList[newProfileListIndex])
                {
                    if (m_gaiaSettings.m_selectedWaterProfile != newProfileListIndex)
                    {
                        ...
                        {
                            m_profile.m_selectedProfile = "None";
                        }
                        else
                        {
                            m_profile.m_selectedProfile = m_profile.m_waterProfiles[newProfileListIndex].m_typeOfWater;
                            m_profileValues = GetProfile();
                            GaiaWater.GetProfile(newProfileListIndex, m_gaiaOceanMat, m_profile, m_renderPipeline, true, true);
                            updateSettingsOnly = false;
                        }

                        EditorUtility.SetDirty(m_gaiaSettings);
                    }
                }
                ...
                EditorGUILayout.BeginHorizontal();
                newProfileListIndex = EditorGUILayout.Popup("Water Profile", newProfileListIndex, m_profileList.ToArray(), GUILayout.MaxWidth(2000f));
                if (m_editorUtils.Button("AddProfile"))
                {
                    int count = m_profile.m_waterProfiles.Count + 1;
                    m_profile.m_waterProfiles.Add(new GaiaWaterProfileValues { m_typeOfWater = "New Profile" + count });
                    newProfileListIndex = m_profileList.Count - 1;
                }
                if (m_profileList[newProfileListIndex] == "None")
                {
                    GUI.enabled = false;
                }
                if (m_editorUtils.Button("RemoveProfile"))
                {
                    m_profile.m_waterProfiles.RemoveAt(newProfileListIndex);
                    if (m_profileList[newProfileListIndex] == "None")
                    {
                        newProfileListIndex = m_profileList.Count - 1;
                    }
                ...
                if (m_profileList[newProfileListIndex] != "None")
                {
                    m_profileValues.m_typeOfWater = m_editorUtils.TextField("ProfileName", m_profileValues.m_typeOfWater, helpEnabled);
                }
        ...
        private GaiaWaterProfileValues GetProfile()
        {
            foreach (GaiaWaterProfileValues profile in m_profile.m_waterProfiles)
            {
                if (profile.m_typeOfWater == m_profile.m_selectedProfile)
                {
                    return profile;
                }
            }
