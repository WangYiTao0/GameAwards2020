...
#region Public API Methods
        ...
        public float GetStamperRange(Terrain currentTerrain)
        {
            ...
            //limit the width to not overstep the max internal resolution value of 4097 of the stamper preview

            float range = Mathf.RoundToInt(Mathf.Clamp(m_settings.m_width, 1, (float)4097 / (float)currentTerrain.terrainData.heightmapResolution * 100));
        ...
        public void AlignToGround()
        {
            ...
            if (t == null)
            {
                target = terrainHeightOffset + m_scanBounds.extents.y;
            }
            else
            {
                float zero = m_scanBounds.min.y + (m_scanBounds.size.y * m_settings.m_baseLevel);
                target = m_scanBounds.center.y - (zero - terrainHeightOffset);
            }

            if (target != m_settings.m_y)
            {
                Debug.Log("Ground base is activated in the stamp location settings, grounding stamp...");
                m_settings.m_y = target;
            }
...
#region Unity Related Methods
        ...
        public void FitToAllTerrains(bool loadedOnly=false) 
        {
            ...
            if (TerrainHelper.GetTerrainBounds(ref b, loadedOnly))
            {
                ...
                if (terrain != null)
                {
                    ...
                    m_settings.m_width = Mathf.RoundToInt(Mathf.Clamp(m_settings.m_width, 1, (float)4097 / (float)terrain.terrainData.heightmapResolution * 100));
                }
                else
                {
#if GAIA_PRO_PRESENT
                    ...
                    if (terrainScene != null)
                    {
                        ...
                        m_settings.m_width = Mathf.RoundToInt(Mathf.Clamp(m_settings.m_width, 1, (float)4097 / (float)m_gaiaSettings.m_currentDefaults.m_heightmapResolution * 100));
                    }
                    else
                    {
#endif
        ...
        public void DrawStampPreview()
        {
            ...
            if ((m_stampDirty || transform.hasChanged) && m_settings != null)
            {
                m_settings.m_x = transform.position.x;
                m_settings.m_y = transform.position.y;
                m_settings.m_z = transform.position.z;
                m_settings.m_rotation = transform.localEulerAngles.y;
                if (transform.localScale.x != m_settings.m_width || transform.localScale.z != m_settings.m_width)
                {
                    float deltaX = Mathf.Abs(transform.localScale.x - m_settings.m_width);
                    float deltaZ = Mathf.Abs(transform.localScale.z - m_settings.m_width);
                    if (deltaX > deltaZ)
                    {
...
#region Private worker methods
        ...
        public RenderTexture ApplyBrushInternal(GaiaMultiTerrainOperation operation)
        {
            ...
            RenderTexture outputTexture = RenderTexture.GetTemporary(operation.m_heightmapPixels.width, operation.m_heightmapPixels.height, 0, Terrain.heightmapRenderTextureFormat, RenderTextureReadWrite.Linear); ;
            if (m_stampDirty || m_cachedRenderTexture == null)
            {
                ...
                localOutputTexture = ImageProcessing.ApplyMaskStack(localinputTexture,m_settings.m_imageMasks, ImageMaskInfluence.Local);
                globalOutputTexture = ImageProcessing.ApplyMaskStack(globalInputTexture, m_settings.m_imageMasks, ImageMaskInfluence.Global);
                ...
                m_cachedMaskTexture = new RenderTexture(operation.RTheightmap);
                Graphics.Blit(globalInputTexture, m_cachedMaskTexture, multiplyMat, 0);


                //bool activeLocalFilters = m_settings.m_imageMasks.Where(x => x.m_active == true && x.m_influence == ImageMaskInfluence.Local).Count()>0;
                //bool activeGlobalFilters = m_settings.m_imageMasks.Where(x => x.m_active == true && x.m_influence == ImageMaskInfluence.Global).Count() > 0;
                ...
                switch (m_settings.m_operation)
                {
                    case GaiaConstants.FeatureOperation.HydraulicErosion:
                        ...
                        m_Eroder.ReleaseRenderTextures();
                        //ImageProcessing.WriteRenderTexture("D:\\ErosionOutputHeight.png", outputTexture);
                    ...
                    case GaiaConstants.FeatureOperation.HeightTransform:
                        ...
                        //else
                        //    heightTransformMat.SetTexture("_BrushTex", localinputTexture);
                    ...
                    default:
                        ...
                        //else
                        //mat.SetTexture("_BrushTex", localinputTexture);


                        //if (activeGlobalFilters)
                        mat.SetTexture("_GlobalMaskTex", globalOutputTexture);
                        //else
                        //    mat.SetTexture("_GlobalMaskTex", Texture2D.whiteTexture);
                ...
                m_cachedRenderTexture = new RenderTexture(outputTexture);
