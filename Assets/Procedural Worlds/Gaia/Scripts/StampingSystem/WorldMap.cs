...
namespace Gaia
{
    public class WorldMap : MonoBehaviour
    {
        ...
        public void CreateWorldMapTerrain()
        {
            ...
            float localHeightmapRes = 1;
            float localTileSize = 0;
            ...
            if (GaiaUtils.HasDynamicLoadedTerrains())
            {
                ...
                {
                    averageHeight += ts.m_bounds.size.y;
                    count++;
                }

                //if we have a dynamic loaded scenario, and there is still no tile size, load in the first terrain scene and grab the size from there
                TerrainScene terrainScene = GaiaTerrainLoaderManager.TerrainScenes[0];
...
#else
                Scene scene = SceneManager.GetSceneByPath(terrainScene.m_scenePath);
#endif
                foreach (GameObject go in scene.GetRootGameObjects())
                {
                    ...
                    if (terrain != null)
                    {
                        localHeightmapRes = terrain.terrainData.heightmapResolution;
                        localTileSize = terrain.terrainData.size.x;
                    }
                }
            ...
            else
            {
                foreach (Terrain t in Terrain.activeTerrains)
                {
                    ...
                    {
                        localHeightmapRes = t.terrainData.heightmapResolution;
                        localTileSize = t.terrainData.size.x;
                    }
            ...
            worldCreationSettings.m_tileHeight = worldCreationSettings.m_tileSize; //(count > 0) ? (float)averageHeight * worldCreationSettings.m_tileSize / (float)bounds.size.x : (float)averageHeight;
            ...
            GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_hasWorldMap = true;

            float worldheightmapRes = m_worldMapTerrain.terrainData.heightmapResolution / m_worldMapTerrain.terrainData.size.x;

            if (bounds.size.x > 0)
            {
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize = (float)worldCreationSettings.m_tileSize / (float)bounds.size.x;
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMapRelativeHeightmapPixels = (((float)bounds.size.x / localTileSize) * localHeightmapRes / localHeightmapRes);
            }
            else
            {
                //no bounds size means no real terrains (yet) - set these parameters according to the current settings/defaults then
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize = (float)worldCreationSettings.m_tileSize / (gaiaSettings.m_tilesX * currentDefaults.m_terrainSize);
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMapRelativeHeightmapPixels = ((gaiaSettings.m_tilesX * currentDefaults.m_terrainSize) / currentDefaults.m_terrainSize) * currentDefaults.m_heightmapResolution / currentDefaults.m_heightmapResolution;
            }
        ...
        public static void ShowWorldMapStampSpawner()
        {
            ...
            if (spawner == null)
            {
                ...
                else
                {
                    ...
                    GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesX = settings.m_tilesX;
                    GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesZ = settings.m_tilesZ;
