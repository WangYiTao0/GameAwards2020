...
namespace Gaia
{
    ...
    public class Spawner : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField]
        private Vector2 m_lastStampSpawnWorldSize;
        ...
        public bool m_massStampRunning;
        ...
        {
            m_lastStampSpawnWorldSize = new Vector2(m_worldCreationSettings.m_xTiles * m_worldCreationSettings.m_tileSize, m_worldCreationSettings.m_zTiles * m_worldCreationSettings.m_tileSize);
        ...
        {
            return m_lastStampSpawnWorldSize != new Vector2(m_worldCreationSettings.m_xTiles * m_worldCreationSettings.m_tileSize, m_worldCreationSettings.m_zTiles * m_worldCreationSettings.m_tileSize);
        }
        ...
        private RenderTexture ApplyBrush(GaiaMultiTerrainOperation operation, MultiTerrainOperationType opType, int spawnRuleID = 0)
        {
            ...
            Graphics.Blit(ImageProcessing.ApplyMaskStack(inputTexture1, biomeControllerStack, ImageMaskInfluence.Local), biomeOutputTexture);

            //Get the combined masks from the spawner
            RenderTexture spawnerOutputTexture = RenderTexture.GetTemporary(rtDescriptor);
            Graphics.Blit(ImageProcessing.ApplyMaskStack(inputTexture2, spawnerStack, ImageMaskInfluence.Local), spawnerOutputTexture);

            //Get the combined masks from the rule
            RenderTexture ruleOutputTexture = RenderTexture.GetTemporary(rtDescriptor);
            Graphics.Blit(ImageProcessing.ApplyMaskStack(inputTexture3, maskStack, ImageMaskInfluence.Local), ruleOutputTexture);
        ...
        public IEnumerator AreaSpawn(List<Spawner> spawners, BoundsDouble area, List<string> validTerrainNames =null)
        {
            ...
            double originalLoadingRange = GaiaTerrainLoaderManager.Instance.GetLoadingRange();
            Vector3Double originalOrigin = GaiaTerrainLoaderManager.Instance.GetOrigin();
            ...
            foreach (Spawner spawner in spawners.Where(x => x.isActiveAndEnabled))
            {
                ...
                foreach (SpawnRule rule in spawner.settings.m_spawnerRules)
                {
                    if (rule.m_isActive)
                    {
                        activeSpawnRules++;
                ...
                for (currentSpawnCenter = new Vector3(currentSpawnCenter.x, currentSpawnCenter.y, startPosition.z); currentSpawnCenter.z <= (area.max.z + spawnRange / 2f); currentSpawnCenter += new Vector3(0f, 0f, locationIncrement))
                {
                    ...
                    foreach (Spawner spawner in spawners)
                    {
                        ...
                        if (GaiaUtils.HasDynamicLoadedTerrains() && !m_settings.m_isWorldmapSpawner)
                        {
                            ...
                            GaiaTerrainLoaderManager.Instance.SetOrigin(currentSpawnCenter);
                            ...
                            GaiaTerrainLoaderManager.Instance.SetLoadingRange(spawnRange - 0.001f);
            ...
            {
                GaiaTerrainLoaderManager.Instance.SetLoadingRange(originalLoadingRange);
                GaiaTerrainLoaderManager.Instance.SetOrigin(originalOrigin);
            }
#endif
            yield return null;
        }
        ...
        private void ExecuteSpawn(GaiaMultiTerrainOperation operation, BakedMaskCache collisionMaskCache, int totalSpawns, ref int totalSpawsCompleted, bool allowStatic = true)
        {
            ...
            int maxSpawnerRules = m_settings.m_spawnerRules.Where(x => x.m_isActive == true).Count();
                        ...
                        switch (m_settings.m_spawnerRules[i].m_resourceType)
                        {
                            ...
                            case GaiaConstants.SpawnerResourceType.TerrainDetail:
                                ...
                                if (m_settings.spawnMode == SpawnMode.Replace)
                                {
                                    ...
                                    {
                                        operation.SetTerrainDetails(tempTerrainDetailRT, spawnMode, this, m_settings.m_spawnerRules[i], randomSeeds[i], false);
                                    }
                                }
                                else
                                {
                                    if (affectedDetailTerrainsCount > 0)
                                    {
                                        operation.SetTerrainDetails(tempTerrainDetailRT, m_settings.spawnMode, this, m_settings.m_spawnerRules[i], randomSeeds[i], false);
                                    }
                                }
                            ...
                            case GaiaConstants.SpawnerResourceType.TerrainTree:
                                ...
                                {
                                    operation.SetTerrainTrees(tempTreeRT, m_settings.spawnMode, this, m_settings.m_spawnerRules[i], randomSeeds[i], false);
                                }
                            ...
                            case GaiaConstants.SpawnerResourceType.GameObject:
                                ...
                                {
                                    operation.SetTerrainGameObjects(tempGameObjectRT, protoGO, m_settings.m_spawnerRules[i], m_settings.spawnMode, randomSeeds[i], ref m_settings.m_spawnerRules[i].m_spawnedInstances, m_settings.m_spawnerRules[i].m_minRequiredFitness, false, true);
                                }
            ...
            if (m_settings.m_isWorldmapSpawner)
            {
                ...
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesX = m_worldCreationSettings.m_xTiles;
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesZ = m_worldCreationSettings.m_zTiles;
...
#region Handy helpers
        ...
        public void ClearStampDistributions(ClearSpawnFor clearSpawnFor, ClearSpawnFrom clearSpawnFrom)
        {
            ...
            foreach (Spawner spawner in allAffectedSpawners)
            {
                GaiaUtils.DisplayProgressBarNoEditor("Clearing Stamps...", "Spawner " + completedSpawners.ToString() + " of " + allAffectedSpawners.Count().ToString(), (float)completedSpawners / (float)allAffectedSpawners.Count());
                foreach (SpawnRule spawnRule in spawner.m_settings.m_spawnerRules)
                {
        ...
        public void ClearGameObjectsForRule(SpawnRule spawnRule, bool allTerrains = true, Terrain terrainToDeleteFrom = null)
        {
            ...
            foreach (Terrain t in relevantTerrains)
            {
                ...
                Transform target = GaiaUtils.GetGOSpawnTarget(spawnRule, protoName, t, GaiaSettings.m_hideObjectsInHierachy);
        ...
        public static void HandleAutoSpawnerStack(List<AutoSpawner> autoSpawners, Transform transform, float range, bool allTerrains, BiomeControllerSettings biomeControllerSettings = null)
        {
            ...
            else
            {
                if (GaiaUtils.HasDynamicLoadedTerrains())
                {
                    spawnArea.center = new Vector3Double(transform.position) + GaiaTerrainLoaderManager.Instance.GetOrigin();
                }
                else
                {
                    spawnArea.center = transform.position;
