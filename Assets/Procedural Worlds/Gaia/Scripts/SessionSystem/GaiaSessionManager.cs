...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public void RegenerateFlaggedTerrains()
        {
            ...
            if (GaiaTerrainLoaderManager.Instance.WorldMapTerrain != null)
            {
...
[UnityEditor.Callbacks.DidReloadScripts]
        ...
        public static GaiaSessionManager GetSessionManager(bool pickupExistingTerrain = false, bool createSession = true)
        {
            ...
            if (mgrObj == null)
            {
                if (createSession)
                {
                    ...
                    if (gaiaSettings != null && sessionMgr.m_session != null)
                    {
                        sessionMgr.SetSeaLevel(gaiaSettings.m_currentDefaults.m_seaLevel);
                    }
        ...
        public void GetWorldMinMax(ref float minWorldHeight, ref float maxWorldHeight, bool isWorldmap = false)
        {
            ...
            if (isWorldmap)
            {
                if (GaiaTerrainLoaderManager.Instance.WorldMapTerrain != null)
                {
                    allTerrains = new Terrain[1] { GaiaTerrainLoaderManager.Instance.WorldMapTerrain };
                }
                else
                {
                    //no world map to analyze height for
                    return;
                }
            }
            else
            {
                allTerrains = Terrain.activeTerrains.Where(x => !TerrainHelper.IsWorldMapTerrain(x)).ToArray();
            }
            minWorldHeight = float.MaxValue;
            maxWorldHeight = float.MinValue;

            //Iterate over all terrains and find the highest min max value
            foreach (Terrain t in allTerrains)
            {
                ...
                if (newMin < minWorldHeight)
                {
                    minWorldHeight = newMin;
                }

                if (newMax > maxWorldHeight)
                {
                    maxWorldHeight = newMax;
                }
...
//                m_session.m_defaults.m_content = PWCommon2.Utils.ReadAllBytes(m_session.m_defaults.m_fileName);
//                SaveSession();
//#endif
//            }
        }
        ...
        public GaiaSession CreateSession(bool pickupExistingTerrain = false)
        {
            ...
            AssetDatabase.CreateAsset(m_session, string.Format(path + "/GS-{0:yyyyMMdd-HHmmss}.asset", DateTime.Now));
            AssetDatabase.SaveAssets();
#endif
            return m_session;
        }
        ...
        public void SetSeaLevel(float seaLevel, bool setFromWorldMap = false)
        {
            ...
            {
                m_session.m_seaLevel = seaLevel / GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize;
            }
        ...
        public float GetSeaLevel(bool getForWorldMap = false)
        {
            ...
            if (getForWorldMap)
            {
                seaLevel *= GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize;
            }
...
//            UnityEngine.Random.InitState(System.Environment.TickCount);
        ...
        public void ContinueMassStamp()
        {
            //Just execute one Stamp per call and increment the index, the stamper will then call this function again when the stamp has been fully processed.
            if (m_massStamperSettingsIndex < m_massStamperSettingsList.Count - 1)
            {
                GaiaSessionManager.Stamp(m_massStamperSettingsList[m_massStamperSettingsIndex], true);
                if (m_massStamperSettingsIndex != m_massStamperSettingsList.Count - 1)
                {
                    m_massStamperSettingsIndex++;
                }
                else
                {
                    //Preventing accidentally stamping again by setting the index to max
        ...
        public void EditStampOperation(GaiaOperation op, StamperSettings stamperSettings)
        {
            ...
            double range = op.StamperSettings.m_width / 100 * GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesSize;
            ...
            operationBounds.center += GaiaTerrainLoaderManager.Instance.GetOrigin();
        ...
        {
            m_originBackup = GaiaTerrainLoaderManager.Instance.GetOrigin();
            m_loadRangeBackup = GaiaTerrainLoaderManager.Instance.GetLoadingRange();
        }

        private void RestoreOriginAndLoadRange()
        {
            GaiaTerrainLoaderManager.Instance.SetOrigin(m_originBackup);
            GaiaTerrainLoaderManager.Instance.SetLoadingRange(m_loadRangeBackup);
        }
        ...
        public static void ContinueSessionPlayback()
        {
            ...
            foreach (GaiaOperation op in sessionManager.m_session.m_operations)
            {
                if (op.m_isActive && op.sessionPlaybackState == SessionPlaybackState.Queued)
                {
                    ...
                    //    sessionManager.m_waitForSessionContinueAfterStamping = true;
                        return;
                    }
        ...
        public static void LoadSession(GaiaSession session = null)
        {
            ...
            //        //operation.serializedObject.ApplyModifiedProperties();
            //        //stamper = (Stamper)operation.serializedObject.targetObject;
            ...
            //        //        as Gaia.GaiaResource;
            //        //}
            //        //Make sure the mask textures are rebuilt
            //        stamper.m_MaskTexturesDirty = true;
            ...
            //        //Grab the embedded resources that were exported and associate them
            //        //spawner.m_settings.m_resources = GaiaUtils.GetAsset(
            //        //    ScriptableObjectWrapper.GetSessionedFileName(m_session.GetSessionFileName(), spawner.m_settings.m_resourcesPath), typeof(Gaia.GaiaResource)) 
            //        //    as Gaia.GaiaResource;

            //        //if (spawner.m_settings.m_resources == null)
            //        //{
            //        //    ExportSessionResource(spawner.m_settings.m_resourcesPath);
            //        //    spawner.m_settings.m_resources = GaiaUtils.GetAsset(
            //        //        ScriptableObjectWrapper.GetSessionedFileName(m_session.GetSessionFileName(), spawner.m_settings.m_resourcesPath), typeof(Gaia.GaiaResource)) 
            //        //        as Gaia.GaiaResource;
            //        //}
            ...
            //            spawner.m_settings.m_resources.ChangeSeaLevel(m_session.m_seaLevel);
            //        //}
            //    }
            ...
            {
                Debug.LogError("The current session is locked for changes, could not create new terrain(s) in this session");
                return;
            }
        ...
        public static void FlattenTerrain(List<string> terrainNames, bool executeNow)
        {
            ...
            if (sessionMgr.IsLocked() == true)
            {
                Debug.LogError("The current session is locked for changes, could not flatten terrain(s) in this session");
                return;
            }
...
//            path = Path.Combine(path, PWCommon2.Utils.FixFileName(m_session.m_name));
...
//                PWCommon2.Utils.WriteAllBytes(exportName, m_session.m_defaults.m_content);
//            }
...
//                PWCommon2.Utils.WriteAllBytes(exportName, kvp.Value.m_content);
//            }
...
#region Terrain Min Max Cache
        private void GetTerrainMinMax(Terrain terrain, ref float min, ref float max, bool forceRecalculate = false)
        {
            ...
            else
            {
                ...
                float newMin = 0f;
                float newMax = 0f;
                GaiaUtils.DisplayProgressBarNoEditor("Calculating heigths", "Calculating the min and max height of the game world...", 0.5f);
        ...
        private float GetMinMaxTerrainHeight(Terrain terrain, ref float min, ref float max)
        {
            ...
            for (int x = 0; x <= terrain.terrainData.heightmapResolution; x++)
            {
                ...
                {
                    float value = terrain.terrainData.GetHeight(x, z);
                    if (value > max)
                        max = value;
                    if (value < min)
                        min = value;
        ...
        private static GameObject CreateTile(int tx, int tz, WorldCreationSettings worldCreationSettings ,ref Terrain[,] world, TerrainLayer[] terrainLayers, DetailPrototype[] terrainDetails, TreePrototype[] terrainTrees)
        {
            ...
            terrain.shadowCastingMode = gaiaDefaults.m_shaodwCastingMode;
#else
            terrain.castShadows = m_castShadows;
#endif
            terrain.detailObjectDensity = gaiaDefaults.m_detailDensity;

            if (gaiaDefaults.m_detailDistance > 250)
            {
            ...
            if (newWorldCreationSettings.m_spawnerPresetList != null)
            {
                foreach (BiomeSpawnerListEntry entry in newWorldCreationSettings.m_spawnerPresetList)
                {
                    if (entry.m_spawnerSettings != null)
                    {
                        ...
                        clonedList.Add(new BiomeSpawnerListEntry() { m_isActive = entry.m_isActive, m_autoAssignPrototypes = entry.m_autoAssignPrototypes, m_spawnerSettings = spawnerSettings });
                    }
                }
            }
                    ...
                    if (stamperSettings.m_imageMasks.Length > 0)
                    {
                        switch (stamperSettings.m_imageMasks[0].m_operation)
                        {
                            case ImageMaskOperation.ImageMask:
                                imageMaskInfo = stamperSettings.m_imageMasks[0].m_imageMaskTexture.name;
                                break;
                            default:
        ...
        public static bool ExecuteOperation(GaiaOperation op)
        {
            ...
            {
                case GaiaOperation.OperationType.CreateWorld:
                    return ExecuteCreateWorld(op.WorldCreationSettings);
                case GaiaOperation.OperationType.Stamp:
                    return ExecuteStamp(op, null);
                case GaiaOperation.OperationType.Spawn:
...
#region OPERATION EXECUTION
        private static bool ExecuteCreateWorld(WorldCreationSettings worldCreationSettings)
        {
            m_regenerateRun = false;
            ...
            bool hasWorldMap = GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_hasWorldMap;

#if UNITY_EDITOR
            string terrainSceneStoragePath = "";
            //Has the original scene been saved before? If yes, delete old potential terrain scene storage
            terrainSceneStoragePath = GaiaDirectories.GetScenePath(sessionMgr.m_session) + "/TerrainScenes.asset";
            if (File.Exists(terrainSceneStoragePath))
            {
                File.Delete(terrainSceneStoragePath);
            }
            
            GaiaTerrainLoaderManager.Instance.ResetStorage();
#endif
            ...
            for (int x = 0; x < worldCreationSettings.m_xTiles; x++)
            {
                for (int z = 0; z < worldCreationSettings.m_zTiles; z++)
                {
                    ...
                    {
#if GAIA_PRO_PRESENT
                        terrainSceneStorage.m_terrainScenes.Add(TerrainSceneCreator.CreateTerrainScene(originalScene, sessionMgr.m_session, terrainGO, worldCreationSettings));
#endif
                    }
                    else
                    {
                        if (worldCreationSettings.m_isWorldMap)
                        {
                            GameObject wme = GaiaUtils.GetOrCreateWorldDesigner();
            ...
            if (sessionMgr.GetSeaLevel() == worldCreationSettings.m_gaiaDefaults.m_seaLevel)
            {
                sessionMgr.SetSeaLevel(worldCreationSettings.m_tileHeight / 2048 * 50);
            }
            ...
            GaiaTerrainLoaderManager.Instance.m_terrainUnloadMemoryTreshold = gaiaSettings.m_terrainUnloadMemoryTreshold;
                ...
                GaiaTerrainLoaderManager.Instance.LoadStorageData();
#if GAIA_PRO_PRESENT
                if (worldCreationSettings.m_createInScene)
                {
                    GaiaTerrainLoaderManager.Instance.SetLoadingRange(Mathf.Round((worldCreationSettings.m_tileSize * 1.9f)/2f));
                    GaiaTerrainLoaderManager.Instance.UpdateTerrainLoadState();
                    AddTerrainScenesToBuildSettings(GaiaTerrainLoaderManager.TerrainScenes);
                }
#endif
#endif
            if (!worldCreationSettings.m_isWorldMap)
        ...
        private static bool ExecuteStamp(GaiaOperation op, GameObject stamperGO, bool isMassStamp = false)
        {
            GaiaSessionManager sessionMgr = GaiaSessionManager.GetSessionManager();
            ...
            GaiaUtils.DisplayProgressBarNoEditor("Stamping", "Stamping operation...", 0);
        ...
        private static bool ExecuteClearWorld()
        {
            ...
            {
                GaiaTerrainLoaderManager.Instance.UnloadAll(true);
                //Remove the entire scene directory -> should take care of scene and terrain data deletion entirely
                FileUtil.DeleteFileOrDirectory(GaiaDirectories.GetTerrainScenePath(sessionMgr.m_session));
                FileUtil.DeleteFileOrDirectory(GaiaDirectories.GetTerrainDataScenePath(sessionMgr.m_session));
                //empty the terrainScene list in the session
                GaiaTerrainLoaderManager.TerrainScenes.Clear();
            }
        ...
        private static bool ExecuteClearSpawns(GaiaOperation op, Spawner spawner = null)
        {
            ...
            else
            {
                ...
                else
                {
                    if (GaiaUtils.HasDynamicLoadedTerrains())
                    {
                        affectedTerrainNames = GaiaTerrainLoaderManager.TerrainScenes.Select(x => x.GetTerrainName()).ToList();
                    }
                    else
                    {
