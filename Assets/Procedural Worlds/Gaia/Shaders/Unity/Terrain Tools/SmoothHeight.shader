    Shader "Hidden/Gaia/SmoothHeight" {
    ...
    SubShader {
		...
		Pass
		{
			...
			float4 SmoothHorizontal(v2f i) : SV_Target
			{
				...
				float transformedStrength = UnpackHeightmap(tex2D(_HeightTransformTex, lerp(height, h, brushStrength)));
				return PackHeightmap(transformedStrength);
